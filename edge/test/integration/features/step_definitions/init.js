...
var environment = config['test_03104'].env;
...
console.log('&test_03104 api: [' +  domains[environment] + ', ' + config['test_03104'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['test_03104'].app, config['test_03104'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['test_03104'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
